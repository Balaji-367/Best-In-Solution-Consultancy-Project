...
const AddDevice = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formDataPayload.append('device_name', formData.deviceName)
      formDataPayload.append('model', formData.modelName)
      formDataPayload.append('serial_no', formData.serialNumber)
      formDataPayload.append('ram', formData.ram)
      formDataPayload.append('memory', formData.memory)
      formDataPayload.append('processor', formData.processor)
      formDataPayload.append('availability', 'available')
      if (formData.devicePhoto) {
        formDataPayload.append('device_photo', formData.devicePhoto)
      }

      await api.post(endpoints.devices.list, formDataPayload, true)
  ...
  const handlePhotoChange = (e) => {
    setFormData({
      ...formData,
      devicePhoto: e.target.files[0] || null
    })
  }
    ...
    <div className="p-6 md:p-8 lg:p-10">
      <div className="max-w-2xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="card space-y-6">
          ...
          {/* RAM */}
          <div>
            <label htmlFor="ram" className="block text-sm font-semibold text-text-primary mb-2">
              RAM <span className="text-text-secondary font-normal">(Optional)</span>
            </label>
            <input
              type="text"
              id="ram"
          ...
          {/* Memory */}
          <div>
            <label htmlFor="memory" className="block text-sm font-semibold text-text-primary mb-2">
              Storage / Memory <span className="text-text-secondary font-normal">(Optional)</span>
            </label>
            <input
              type="text"
              id="memory"
              ...
              onChange={handleChange}
              placeholder="e.g., 256GB SSD, 512GB NVMe"
              className="input-field"
              disabled={isSubmitting}
            />
          </div>
          ...
          {/* Processor */}
          <div>
            <label htmlFor="processor" className="block text-sm font-semibold text-text-primary mb-2">
              Processor <span className="text-text-secondary font-normal">(Optional)</span>
            </label>
            <input
              type="text"
              id="processor"
              ...
              onChange={handleChange}
              placeholder="e.g., Intel Core i5 12th Gen, AMD Ryzen 5"
              className="input-field"
              disabled={isSubmitting}
            />
          </div>
          ...
          {/* Device Photo */}
          <div>
            <label htmlFor="devicePhoto" className="block text-sm font-semibold text-text-primary mb-2">
            ...
            {formData.devicePhoto && (
              <p className="mt-2 text-sm text-text-secondary">Selected: {formData.devicePhoto.name}</p>
            )}
            <p className="mt-1 text-xs text-text-secondary">
              Upload a photo of the device. Supported formats: JPG, PNG, GIF.
            </p>
          </div>
