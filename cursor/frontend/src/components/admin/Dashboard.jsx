...
const UserList = ({ type = 'all' }) => {
  ...
  const fetchUsers = async () => {
    try {
      setLoading(true)
      const usersData = await api.get(endpoints.users.list)
      if (type === 'admin') {
...
const getAvatarBg = () => {
    ...
    if (type === 'employee') return 'bg-orange-500'
    return 'bg-primary'
  }
  ...
  const getRoleBadge = (userRole) => {
    if (userRole === 'admin') {
      return <span className="px-2 py-1 bg-red-100 text-red-700 text-xs rounded-full font-medium">Admin</span>
    }
    return <span className="px-2 py-1 bg-blue-100 text-blue-700 text-xs rounded-full font-medium">Employee</span>
  }
  ...
  if (loading) {
    return (
      <div className="text-center py-4">
        ...
        <p className="text-text-secondary mt-2">Loading {type}s...</p>
      </div>
    )
  }
  ...
  if (error) {
    return (
      <div className="text-center py-4">
        ...
        <button onClick={fetchUsers} className="text-primary hover:underline mt-2">
          Retry
        </button>
      </div>
    )
    ...
    <div>
      ...
      {filteredUsers.length === 0 ? (
        <div className="text-center py-8">
          ...
          <p className="text-text-secondary">No {type}s found</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredUsers.map((user) => (
            <div
              key={user.id}
              className="p-4 bg-gradient-to-r from-gray-50 to-gray-100/50 rounded-xl hover:shadow-md transition-all"
            >
              <div className="flex items-center gap-3">
                <div className={`w-12 h-12 ${getAvatarBg()} rounded-full flex items-center justify-center text-white font-bold text-lg`}>
                  {user.username?.charAt(0).toUpperCase()}
                </div>
                <div className="flex-1 min-w-0">
                  <h4 className="font-semibold text-text-primary truncate">{user.username}</h4>
...
const AdminList = () => <UserList type="admin" />
const EmployeeList = () => <UserList type="employee" />
    ...
    <div className="p-6 md:p-8 lg:p-10 page-transition">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          {/* Quick Actions */}
          <div className="card card-gradient lg:col-span-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <button
                ...
                className="p-4 bg-white rounded-xl hover:shadow-md transition-all duration-200 text-left group"
              >
                ...
                <p className="text-xs text-text-secondary mt-1">Create a service request</p>
              </button>
              ...
              >
                <div className="w-10 h-10 bg-red-100 rounded-lg flex items-center justify-center mb-3 group-hover:bg-red-200 transition-colors">
                  <svg className="w-5 h-5 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15a3 3 0 100-6 0 3 3 0 016 0z M19.4 15a1.6 1.6 0 00-1.1 1.9c.1.5-.1.7 1.4l-.3 1.2a1.6 1.6 0 01-1.6 1.2 1.6 1.6 0 01-1.2-1.6l1.2-.3c.2-.6-.2-1.3-.7-1.4a1.6 1.6 0 00-1.9 1.1v.3a1.6 1.6 0 001.2 1.6l-.3 1.2a1.6 1.6 0 01-1.6 1.2 1.6 1.6 0 01-1.2-1.6l1.2-.3c.2-.6-.2-1.3-.7-1.4a1.6 1.6 0 00-1.9 1.1v.3" />
                  </svg>
                </div>
                <p className="font-semibold text-text-primary">Manage Users</p>
                <p className="text-xs text-text-secondary mt-1">Add admin/employee</p>
              </button>
              ...
              >
                ...
                <p className="font-semibold text-text-primary">Add Device</p>
              ...
              >
                ...
                <p className="font-semibold text-text-primary">AI Analytics</p>
                <p className="text-xs text-text-secondary mt-1">View insights</p>
              </button>
            </div>
          </div>
        </div>
